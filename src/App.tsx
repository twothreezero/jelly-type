    ...
    <Box sx={{ display: "flex", alignItems: "center" }}>
      {keys.map((k, i) => (
        <React.Fragment key={i}>
          ...
          {i < keys.length - 1 && (
            <Typography variant="caption" sx={{ fontSize: "0.6rem", fontWeight: 900 }}>
              +
            </Typography>
          )}
        </React.Fragment>
...
const App: React.FC = () => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Backspace") {
      ...
      if (word === "RAIN" && physicsRef.current) {
        physicsRef.current.triggerRain(getThemeColor);
        setInputValue("");
      } else if (word === "BOMB" && physicsRef.current) {
        physicsRef.current.triggerBomb();
        setInputValue("");
      } else if (word === "LOVE" && physicsRef.current) {
        physicsRef.current.triggerLove();
        setInputValue("");
        ...
        physicsRef.current?.applyGlobalForce();
      }
      e.preventDefault();
      ...
      ["Minus", "Equal", "BracketLeft", "BracketRight", "Semicolon", "Quote", "Comma", "Period", "Slash"].includes(e.code)
    ) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = Array.from(val)
      .filter((char) => !/[ㄱ-ㅎㅏ-ㅣ가-힣]/.test(char))
      .join("");
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      >
        {blobs.map((blob) => (
          <SmartBackgroundBlob key={blob.id} {...blob} mouseX={mousePos.x} mouseY={mousePos.y} />
        ))}
        <Box
          ref={containerRef}
          id="canvas-container"
          sx={{ position: "absolute", inset: 0, zIndex: 2, "& canvas": { display: "block", width: "100%", height: "100%" } }}
        />
          ...
          >
            <GlassPaper sx={{ p: 0.5, px: isMobile ? 0.5 : 1.5 }}>
              <Stack direction="row" spacing={isMobile ? 0.5 : 1} alignItems="center">
                {isMobile ? (
                  <>
                    <IconButton onClick={(e) => setAppsAnchor(e.currentTarget)} size="small" sx={{ color: "#666" }}>
                      <AppsIcon />
                    </IconButton>
                    <Menu anchorEl={appsAnchor} open={Boolean(appsAnchor)} onClose={() => setAppsAnchor(null)}>
                      ...
                      >
                        <HelpIcon sx={{ mr: 1.5, fontSize: 20 }} /> How to use
                      </MenuItem>
                      <MenuItem
                        onClick={(e) => {
                      ...
                      >
                        <FontIcon sx={{ mr: 1.5, fontSize: 20 }} /> Fonts
                      </MenuItem>
                      <MenuItem
                        onClick={() => {
                          toggleMute();
                      ...
                      >
                        {isMuted ? <VolOffIcon sx={{ mr: 1.5, fontSize: 20 }} /> : <VolOnIcon sx={{ mr: 1.5, fontSize: 20 }} />}{" "}
                        {isMuted ? "Unmute" : "Mute"}
                      </MenuItem>
                      <MenuItem
                        onClick={() => {
                      ...
                      >
                        <DownloadIcon sx={{ mr: 1.5, fontSize: 20 }} /> Save PNG
                      </MenuItem>
                      <MenuItem
                        onClick={() => {
                      ...
                      >
                        <TuneIcon sx={{ mr: 1.5, fontSize: 20 }} /> Settings
                      </MenuItem>
                      <MenuItem
                        onClick={() => {
                          physicsRef.current?.clearAll();
                      ...
                      >
                        <RefreshIcon sx={{ mr: 1.5, fontSize: 20 }} /> Clear All
                      </MenuItem>
                    </Menu>
                  </>
                ) : (
                  <>
                    <Tooltip title="How to use">
                      <IconButton onClick={() => setShowHelp(true)} size="small" sx={{ color: "#666" }}>
                        <HelpIcon />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title="Fonts">
                      <IconButton onClick={(e) => setFontAnchor(e.currentTarget)} size="small" sx={{ color: "#666" }}>
                        <FontIcon />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title={isMuted ? "Unmute" : "Mute"}>
                      <IconButton onClick={toggleMute} size="small" sx={{ color: isMuted ? "#f44336" : "#666" }}>
                        {isMuted ? <VolOffIcon /> : <VolOnIcon />}
                      </IconButton>
                    </Tooltip>
                    <Tooltip title={isGyroActive ? "Gyro Active" : "Gyro Off"}>
                      <IconButton
                        onClick={toggleGyro}
                        size="small"
                        sx={{ color: isGyroActive ? "#000" : "#666", bgcolor: isGyroActive ? "rgba(0,0,0,0.05)" : "transparent" }}
                      >
                        <GyroIcon />
                      </IconButton>
                    </Tooltip>
                    ...
                    <Tooltip title="Save PNG (Ctrl+S)">
                      <IconButton onClick={exportPng} size="small" sx={{ color: "#666" }}>
                        <DownloadIcon />
                      </IconButton>
                    </Tooltip>
                    <Box sx={{ width: "1px", height: 24, bgcolor: "rgba(0,0,0,0.1)", mx: 0.5 }} />
                    <Tooltip title="Settings">
                      <IconButton onClick={() => setShowSettings(true)} size="small" sx={{ color: "#666" }}>
                        <TuneIcon />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title="Clear All (Ctrl+Backspace)">
                      <IconButton
                        onClick={() => {
                          physicsRef.current?.clearAll();
                          setInputValue("");
          ...
          >
            <AnimatePresence mode="wait">
              ...
              >
                    ...
                    endAdornment: (
                      <InputAdornment position="end">
                        <IconButton
                          ...
                          size="small"
                          sx={{ color: THEMES[activeTheme].mainColor, transition: "all 0.4s ease", mr: 0.5 }}
                        >
                          <PaletteIcon />
                        </IconButton>
                      </InputAdornment>
                ...
                <Typography
                  variant="body2"
                  sx={{ mt: 1.5, color: "#999", fontSize: isMobile ? "0.75rem" : "0.875rem", fontWeight: 600, fontFamily: "'Titan One', sans-serif" }}
                >
        ...
        <Menu anchorEl={fontAnchor} open={Boolean(fontAnchor)} onClose={() => setFontAnchor(null)}>
          {(Object.keys(FONTS) as FontType[]).map((f) => (
            <MenuItem
              ...
              selected={activeFont === f}
              sx={{ fontFamily: FONTS[f] }}
            >
              {f}
            </MenuItem>
          ))}
        ...
        <Modal open={showHelp} onClose={() => setShowHelp(false)}>
          <Box
            sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: isMobile ? 320 : 400, outline: "none" }}
          >
            <GlassPaper sx={{ p: 4, background: "rgba(255,255,255,0.95)", color: "#333" }}>
              <Stack spacing={2.5}>
                <Typography variant="h6" align="center" sx={{ fontWeight: 900, fontFamily: "'Titan One', sans-serif" }}>
                  Shortcuts & Manual
                </Typography>

                <Box>
                  <Typography variant="subtitle2" sx={{ fontWeight: 800, mb: 1, color: "#FF6B6B" }}>
                    Keyboard
                  </Typography>
                  <Stack spacing={1}>
                ...
                <Box>
                  <Typography variant="subtitle2" sx={{ fontWeight: 800, mb: 1, color: "#4facfe" }}>
                    Interaction
                  </Typography>
                  <Stack spacing={1}>
                    <Stack direction="row" spacing={2}>
                      ...
                      <Typography variant="caption" sx={{ ml: 1 }}>
                        Click & drag to play with jellies.
                      </Typography>
                    </Stack>
                    <Stack direction="row" spacing={2}>
                      <MouseIcon fontSize="small" />
                      <Typography variant="caption" sx={{ ml: 1 }}>
                        <b>Right-click anywhere</b> to toggle Magnet.
                      </Typography>
                    </Stack>
                    <Stack direction="row" spacing={2}>
                      <MagnetIcon fontSize="small" />
                      <Typography variant="caption" sx={{ ml: 1 }}>
                        Click Magnet icon to Lock position.
                      </Typography>
                    </Stack>
                  </Stack>
                </Box>

                <IconButton onClick={() => setShowHelp(false)} sx={{ alignSelf: "center", mt: 1 }}>
                  <CloseIcon />
                </IconButton>
              </Stack>
            </GlassPaper>
        ...
        <Drawer
          ...
          PaperProps={{ sx: { width: isMobile ? "80%" : 300, background: "rgba(255, 255, 255, 0.9)", backdropFilter: "blur(30px)", p: 3 } }}
        >
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 4 }}>
            <Typography variant="h6" sx={{ fontFamily: "'Titan One', sans-serif" }}>
              Settings
            </Typography>
          ...
          <SettingRow
            ...
            max={2}
            step={0.1}
            onChange={(v: number) => handleConfigChange("gravityY", v)}
          />
          <SettingRow
            label="Bounciness"
            ...
            max={1.2}
            step={0.05}
            onChange={(v: number) => handleConfigChange("restitution", v)}
          />
          <SettingRow
            label="Friction"
            ...
            max={1}
            step={0.01}
            onChange={(v: number) => handleConfigChange("friction", v)}
          />
          <SettingRow
            ...
            max={0.2}
            step={0.001}
            onChange={(v: number) => handleConfigChange("airResistance", v)}
          />
          <Box sx={{ mt: 4, pt: 2, borderTop: "1px solid rgba(0,0,0,0.1)" }}>
            <Button
              ...
              sx={{ borderRadius: "12px", color: "#666", borderColor: "#ccc", fontFamily: "'Titan One', sans-serif" }}
            >
              Reset to Default
            </Button>
          </Box>
